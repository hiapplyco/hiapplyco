...
const AgentEcosystemVisualization = () => {
  ...
  const ToolNode = ({
    ...
    return <div className={`transition-all duration-700 delay-[${delay}ms] ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        <div className="relative glass p-3 rounded-lg w-40 h-40 flex flex-col items-center justify-center border border-border/40">
          ...
          {services.length > 0 && <div className="mt-1">
              <div className="flex flex-wrap justify-center gap-1">
                {services.map((service, idx) => <span key={idx} className="bg-secondary/60 text-[10px] px-1 py-0.5 rounded">
                    {service}
                  </span>)}
              </div>
            </div>}
  ...
  return <section id="ecosystem" className="section-spacing px-6 relative overflow-hidden">
      ...
      <div className="max-w-5xl mx-auto" ref={visualizationRef}>
        ...
        <div className="w-full overflow-x-auto pb-6">
          <div className="flex flex-nowrap min-w-[900px] justify-center items-center p-4">
            {/* Hub */}
            <ToolNode icon={Database} title="Central Hub" color="blue" services={["Orchestration"]} delay={0} />
            
            <ConnectionLine delay={150} type="data" />
            
            {/* Executive Assistant */}
            <ToolNode icon={Calendar} title="Executive Assistant" color="purple" services={["Calendar", "Email"]} delay={200} />
            
            <ConnectionLine delay={350} />
            
            {/* Customer Service */}
            <ToolNode icon={MessageSquare} title="Customer Service" color="green" services={["Support", "Chat"]} delay={400} />
            
            <ConnectionLine delay={550} type="data" />
            
            {/* Data Analysis */}
            <ToolNode icon={PieChart} title="Data Analysis" color="yellow" services={["Reports", "Insights"]} delay={600} />
            
            <ConnectionLine delay={750} />
            
            {/* Content Creation */}
            <ToolNode icon={Users} title="Content Creation" color="red" services={["Marketing", "Social"]} delay={800} />
          </div>
          ...
          <div className={`mt-4 glass rounded-lg p-4 transition-all duration-1000 delay-[900ms] ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            ...
            <div className="grid grid-cols-5 gap-2">
              ...
              {[["Google", "Microsoft", "Slack"], ["Zoom", "Asana", "Notion"], ["Zendesk", "Intercom", "HubSpot"], ["QuickBooks", "Tableau", "Google Analytics"], ["Canva", "Buffer", "WordPress"]].map((toolGroup, groupIdx) => <div key={groupIdx} className="flex flex-col items-center">
                  <div className="h-6 border-l border-border/50 w-0"></div>
                  <div className="bg-secondary/30 p-2 rounded w-full">
                    {toolGroup.map((tool, idx) => <div key={idx} className="text-[10px] text-center mb-1 bg-background/50 rounded py-0.5">
                        {tool}
                      </div>)}
                  </div>
                </div>)}
